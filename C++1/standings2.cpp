...
// Валидация даты с проверкой дней в месяце
...
public:
    ...
    void print() const {
        cout << date << " | "
             << team1 << " vs " << team2 << " | "
             << (result == 'V' ? "Победа " + team1 : "Ничья между " + team1 + " и " + team2);
    }
...
public:
    ...
    void loadFromFile(const string& filename) {
        ...
        while (getline(file, line)) {
            ...
            if (line[0] == '#') {
                ...
                // Проверка формата первой команды
                if (ss.peek() != '\"') {
                    cerr << "Ошибка формата матча (ожидалась кавычка для первой команды): " << line << endl;
                    continue;
                }
                ss.get(); // пропустить открывающую кавычку
                getline(ss, team1, '\"');
                
                // Пропуск пробелов
                ss >> ws;
                
                // Проверка формата второй команды
                if (ss.peek() != '\"') {
                    cerr << "Ошибка формата матча (ожидалась кавычка для второй команды): " << line << endl;
                    continue;
                }
                ss.get(); // пропустить открывающую кавычку
                getline(ss, team2, '\"');
                ...
                // Обработка формата с кавычками
                if (line[0] == '\"') {
                    size_t end_quote = line.find('\"', 1);
                    if (end_quote != string::npos) {
                        name = line.substr(1, end_quote - 1);
                        string rest = line.substr(end_quote + 1);
                        stringstream ss(rest);
                        ss >> pos >> wins >> draws >> losses;
                    }
                } else {
                    // Обработка старого формата без кавычек
                    ...
                    if (tokens.size() < 5) {
                        cerr << "Ошибка: в строке команды '" << line << "' ожидалось минимум 5 полей. Пропускаем.\n";
                        continue;
                    }

                    // Парсинг последних 4 чисел
                    try {
                        size_t idx = tokens.size() - 4;
                        pos = stoul(tokens[idx]);
                        wins = stoul(tokens[idx+1]);
                        draws = stoul(tokens[idx+2]);
                        losses = stoul(tokens[idx+3]);
                    } catch (...) {
                        cerr << "Ошибка: не удалось преобразовать числа в строке: " << line << ". Пропускаем.\n";
                        continue;
                    }

                    // Сбор названия команды
    ...
    void updateRoundResults() {
        ...
        for (unsigned int i = 0; i < matchCount; ++i) {
            ...
            if (team1 == team2) {
                cerr << "Ошибка: команда не может играть сама с собой! Матч пропущен.\n";
                continue;
            }
            ...
            for (const auto& m : matches) {
                if (m.getDate() == date && 
                    ((m.getTeam1() == team1 && m.getTeam2() == team2) || 
                     (m.getTeam1() == team2 && m.getTeam2() == team1))) {
                    duplicate = true;
                    break;
                }
            }
            ...
            if (duplicate) {
                cout << "Матч между этими командами в эту дату уже существует! Пропускаем...\n";
                continue;
            }
        ...
        sortTeams();
        cout << "\nРезультаты тура обновлены. Успешно добавлено матчей: " << addedCount << "\n";
    }
